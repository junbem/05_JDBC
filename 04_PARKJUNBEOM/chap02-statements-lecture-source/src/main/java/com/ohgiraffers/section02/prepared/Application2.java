...
/*
* PreparedStatement는 Statement와 달리 최초 한 번 쿼리를 파싱하고 컴파일하고 캐싱하여 다시 재해헉하는
...
public class Application2 {
    public static void main(String[] args) {
        ...
        String query = "SELECT MENU_NAME, MENU_PRICE FROM tbl_menu WHERE MENU_CODE = ?";    // 불완전한 쿼리문을 캐싱(저장)

        try {
            pstmt = con.prepareStatement(query);    // 미완성된 쿼리를 이용해서 Prepared객체를 생성
            pstmt.setInt(1, menuCode);  // 위치홀더(?)가 나열된 순서 왼족 ->오른쪽 순서대로 채워나간다.
